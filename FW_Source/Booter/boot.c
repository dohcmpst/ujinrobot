...
void BootComCanInit(void);
void BootComCanCheckActivationRequest(uint8_t *RxData, uint32_t DLC);
...
{
	/* App 초기 과정에서 실행됨으로써 별도로 설정할 필요 없음 */
} /*** end of BootComInit ***/
...
void BootComCanCheckActivationRequest(uint8_t *RxData, uint32_t DLC)
{
	/* check if this was an XCP CONNECT command */
	printf("DLC = %ld, ", DLC);
	for( int k = 0; k < 8; k++) printf("%02X ", RxData[k]);
	printf("\n");
	if ((RxData[0] == 0xff) && (DLC == 2))
	{
		/* connection request received so start the bootloader */
		printf("\n\n.............Booter Requested ! .........\n\n");
		BootActivate();
