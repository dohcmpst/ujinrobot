...
void CanInit_CAN1(void);
void CanInit_CAN2(void);
...
#if (BOOT_COM_CAN_CHANNEL_INDEX == 1)
...
void CanInit_CAN2(void)
{
  ...
  if ((rxMsgId & 0x80000000) == 0)
  {
    rxFilterId = rxMsgId << CAN_RI1R_STID_Pos;
    rxFilterMask = (CAN_RI1R_STID_Msk) | CAN_RI1R_IDE;
  }
  else
  {
    /* negate the ID-type bit */
    rxMsgId &= ~0x80000000;
    rxFilterId = (rxMsgId << CAN_RI1R_EXID_Pos) | CAN_RI1R_IDE;
    rxFilterMask = (CAN_RI1R_EXID_Msk) | CAN_RI1R_IDE;
  }
  /* configure the reception filter. note that the implementation of this function
   * always returns HAL_OK, so no need to evaluate the return value.
   */
...
void CanInit_CAN2(void)
{
  ...
  blt_int8u  tseg1 = 0, tseg2 = 0;
  CAN_FilterTypeDef filterConfig;
  blt_int32u rxMsgId = BOOT_COM_CAN_RX_MSG_ID;      // #define BOOT_COM_CAN_RX_MSG_ID          (0x667 /*| 0x80000000*/)
  blt_int32u rxFilterId, rxFilterMask;
  ...
  filterConfig.FilterMaskIdHigh = 0x0000;  // 0x00000000 = 모든 ID를 받아들이겠다
...
{
  blt_int32u txMsgId = BOOT_COM_CAN_TX_MSG_ID;      // #define BOOT_COM_CAN_TX_MSG_ID          (0x7E1  /*| 0x80000000*/)
...
blt_bool CanReceivePacket(blt_int8u *data, blt_int8u *len)
{
    ...
    HAL_StatusTypeDef canRdFlag;
    static blt_int32u disCnt = 0;
    
    canRdFlag = HAL_CAN_GetRxMessage(&canHandle, CAN_RX_FIFO0, &rxMsgHeader, data);
